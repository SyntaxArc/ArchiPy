...
### How do I rate-limit FastAPI endpoints?

ArchiPy does not ship a FastAPI rate-limit handler. Use the official
[fastapi-redis-sdk](https://github.com/redis/fastapi-redis-sdk) (`rate_limit()` dependency,
`REDIS_*` / `REDIS_URL` env vars). See
[Interceptors — FastAPI Rate Limiting](../tutorials/helpers/interceptors.md#fastapi-rate-limiting).

**Still supported in ArchiPy:** the gRPC rate-limit interceptor (`GrpcServerRateLimitInterceptor`,
`AsyncGrpcServerRateLimitInterceptor`), `grpc_rate_limit_decorator`, `GrpcRateLimitConfig`,
`RateLimitUtils`, and `RateLimitWindowDTO`.
...
- [Observability](../tutorials/observability.md) — OpenTelemetry traces, metrics, and logs
- [Interceptors](../tutorials/helpers/interceptors.md) — gRPC exception and rate-limit interceptors

...
class AlreadyExistsError(BaseError):
    ...
    grpc_status: ClassVar[int] = (
        ...
        if GRPC_AVAILABLE and isinstance(StatusCode.ALREADY_EXISTS.value, tuple)
        else (StatusCode.ALREADY_EXISTS.value if GRPC_AVAILABLE else 6)
    )
...
class DataLossError(BaseError):
    ...
    grpc_status: ClassVar[int] = (
        ...
        if GRPC_AVAILABLE and isinstance(StatusCode.DATA_LOSS.value, tuple)
        else (StatusCode.DATA_LOSS.value if GRPC_AVAILABLE else 15)
    )
